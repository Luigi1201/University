...
//AGGIUNTE DA ME PER ESERCIZIO:
lista listDelFirstEl(lista);
lista listDelInsertEl(lista,int);
//--------------------------------
...
lista lista_append(lista a, float v){
  ...
  /* CANCELLA LISTA 
  *  Funzione1->listDelFirstEl
  *  Funzione2->listDelInsertEl
  */
lista listDelFirstEl(lista a){
	if(a==NULL){
		return a;
	}
	if(a->next==NULL){
...
lista listDelInsertEl(lista a,int pos){
	//eliminiamo l'elemento in posizione pos dalla lista
	lista p = lista_search(a, pos-1);
	lista_print(a);
	lista_print(p);
